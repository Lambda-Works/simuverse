...
exports.TechSheetAnalysisService = void 0;
...
class TechSheetAnalysisService {
    /**
     ...
     */
    async analyzeAndSave(sheet, courseId) {
        try {
            ...
            const tasks = await this.createTasksForKPIs(courseId, kpis, sheet.name);
            ...
            try {
                const existingConfig = await connection_1.AppDataSource.query(`SELECT id FROM course_config WHERE course_id = ?`, [courseId]);
                const metadata = JSON.stringify({
                    analyzed_kpis_config: analyzedConfig
                });
                if (existingConfig.length > 0) {
                    const configId = existingConfig[0].id;
                    await connection_1.AppDataSource.query(`UPDATE course_config SET metadata = JSON_SET(COALESCE(metadata, '{}'), '$.analyzed_kpis_config', CAST(? AS JSON)) WHERE id = ?`, [JSON.stringify(analyzedConfig), configId]);
                    console.log(`✅ Metadata actualizada para config ${configId}`);
                }
                else {
                    const id = (0, uuid_1.v4)();
                    const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
                    await connection_1.AppDataSource.query(`INSERT INTO course_config (id, course_id, config_data, metadata, prompt_generation_mode, tech_sheet_id, created_at, updated_at) 
             VALUES (?, ?, ?, ?, 'template', ?, ?, ?)`, [
                        id,
                        courseId,
                        JSON.stringify({}),
     ...
     */
    async readTechSheetContent(sheet) {
        let content = '';
     ...
     */
    async analyzeWithGemini(content, sheetName) {
  ...
  "kpis": [
    {
      ...
      "evaluation_questions": ["Pregunta?"]
            ...
            const competencies = (parsedData.competencies || []).map((c, idx) => ({
                id: c.id || `comp-${idx + 1}`,
                name: c.name || `Competencia ${idx + 1}`,
                description: c.description || '',
                level: c.level || 'intermediate'
            }));
            const kpis = (parsedData.kpis || []).map((k, idx) => ({
                id: k.id || `kpi-${idx + 1}`,
     ...
     */
    extractCompetenciesAndKPIsBasic(content) {
     ...
     */
    extractCompetenciesFromStructure(content) {
        const competencies = [];
        const lowerContent = content.toLowerCase();
        // Patrones a buscar en el documento
        const patterns = [
            { keyword: 'competencia', level: 'intermediate', baseName: 'Competencia' },
            { keyword: 'habilidad', level: 'intermediate', baseName: 'Habilidad' },
            { keyword: 'destreza', level: 'intermediate', baseName: 'Destreza' },
            { keyword: 'capacidad', level: 'intermediate', baseName: 'Capacidad' },
            { keyword: 'conocimiento', level: 'basic', baseName: 'Conocimiento' },
            { keyword: 'liderazgo', level: 'advanced', baseName: 'Liderazgo' },
            { keyword: 'análisis', level: 'intermediate', baseName: 'Análisis crítico' },
            { keyword: 'comunicación', level: 'intermediate', baseName: 'Comunicación efectiva' },
            { keyword: 'resolución', level: 'advanced', baseName: 'Resolución de problemas' },
        ];
     ...
     */
    extractObjectivesFromStructure(content) {
        const objectives = [];
     ...
     */
    extractModulesFromStructure(content) {
        const modules = [];
     ...
     */
    createKPIsFromObjectivesAndModules(objectives, modules, competencies) {
        const kpis = [];
        let kpiIdx = 1;
     ...
     */
    extractSection(content, keyword, maxLength = 500) {
        const lowerContent = content.toLowerCase();
     ...
     */
    extractCompetencies(contentText) {
        const competencies = [];
        const processedKeywords = new Set();
        // Palabras clave de competencias comunes en fichas técnicas ministeriales
        const competencyKeywords = {
            'gestión': { name: 'Gestión de operaciones', level: 'intermediate' },
            'administración': { name: 'Administración de recursos', level: 'intermediate' },
            'análisis': { name: 'Análisis crítico', level: 'intermediate' },
            'comunicación': { name: 'Comunicación efectiva', level: 'intermediate' },
            'liderazgo': { name: 'Liderazgo', level: 'advanced' },
            'resolución': { name: 'Resolución de problemas', level: 'intermediate' },
            'toma de decisiones': { name: 'Toma de decisiones', level: 'advanced' },
            'planificación': { name: 'Planificación estratégica', level: 'advanced' },
            'evaluación': { name: 'Evaluación de desempeño', level: 'intermediate' },
            'cliente': { name: 'Atención al cliente', level: 'basic' },
            'negociación': { name: 'Negociación', level: 'intermediate' },
            'trabajo en equipo': { name: 'Trabajo en equipo', level: 'basic' },
            'ética': { name: 'Ética profesional', level: 'basic' },
            'calidad': { name: 'Aseguramiento de calidad', level: 'intermediate' },
        };
     ...
     */
    extractKPIs(contentText, competencies) {
        const kpis = [];
        const textLower = contentText.toLowerCase();
     ...
     */
    detectKPICategory(lineLower) {
        if (lineLower.includes('cliente') || lineLower.includes('satisfacción'))
            return 'customer_satisfaction';
        if (lineLower.includes('tiempo') || lineLower.includes('minutos') || lineLower.includes('horas'))
            return 'time';
        if (lineLower.includes('calidad') || lineLower.includes('porcentaje') || lineLower.includes('%'))
            return 'quality';
        if (lineLower.includes('costo') || lineLower.includes('precio') || lineLower.includes('dinero'))
            return 'cost';
        return 'performance';
    }
    /**
     ...
     */
    async createTasksForKPIs(courseId, kpis, techSheetName) {
        const tasks = [];
        const kpiRepo = connection_1.AppDataSource.getRepository(KPI_1.KPI);
        const taskRepo = connection_1.AppDataSource.getRepository(Task_1.Task);
        ...
        for (const kpiConfig of kpis) {
            try {
                ...
                const taskDefs = [
                    { type: 'practice', seq: 1, suffix: 'Práctica' },
                    { type: 'evaluation', seq: 2, suffix: 'Evaluación' }
                ];
                for (const taskDef of taskDefs) {
                    try {
                        const taskEntity = new Task_1.Task();
                        taskEntity.id = (0, uuid_1.v4)();
     ...
     */
    generateSystemPrompt(sheet, kpis) {
        const kpisList = kpis
...
Context de la simulación: ${sheet.description || 'N/A'}`;
     ...
     */
    generateEvaluationPrompt(kpis) {
        const criteria = kpis
            .map((k) => `- ${k.name} (mínimo ${k.minimum_pass_value}%, objetivo ${k.target_value}%)`)
            .join('\n');
...
Intenta esto: [tip 1], [tip 2]. ¿Quieres intentar de nuevo?"`;
     ...
     */
    async getAnalyzedConfig(courseId) {
        try {
            const result = await connection_1.AppDataSource.query(`SELECT metadata FROM course_config WHERE course_id = ?`, [courseId]);
     ...
     */
    async updateAnalyzedConfig(courseId, updates) {
        try {
            ...
            const configQuery = await connection_1.AppDataSource.query(`SELECT id FROM course_config WHERE course_id = ?`, [courseId]);
            if (configQuery.length === 0) {
                throw new Error('CourseConfig not found');
            }
            const configId = configQuery[0].id;
            await connection_1.AppDataSource.query(`UPDATE course_config SET metadata = JSON_SET(COALESCE(metadata, '{}'), '$.analyzed_kpis_config', CAST(? AS JSON)) WHERE id = ?`, [JSON.stringify(updated), configId]);
...
exports.TechSheetAnalysisService = TechSheetAnalysisService;
